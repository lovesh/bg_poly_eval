...
impl UnivarPolyEvalArgProtocol {
    ...
    pub fn init_with_given_commitments(poly: &UnivarPolynomial, variable: FieldElement, evaluation: FieldElement, blinding_variable: FieldElement, blinding_evaluation: FieldElement, comm_variable: G1, comm_evaluation: G1, comm_key: &CommitmentKey) -> UnivarPolyEvalArgProtocol {
        ...
        let comm_even_powers_u = even_powers_u.iter().zip(blindings_even_powers_u.iter()).map(|(u_i, r_i)| comm_key.commit(u_i, r_i)).collect::<Vec<G1>>();
        ...
        let comm_f = f.iter().zip(s.iter()).map(|(f_i, s_i)| comm_key.commit(f_i, s_i)).collect::<Vec<G1>>();
        ...
        let comm_delta = delta.iter().zip(blindings_delta.iter()).map(|(d_i, e_i)| comm_key.commit(d_i, e_i)).collect::<Vec<G1>>();
        ...
        let comm_f_times_u_power = f_times_u_power.iter().zip(blindings_f_times_u.iter()).map(|(f_u_i, e_i)| comm_key.commit(f_u_i, e_i)).collect::<Vec<G1>>();
    ...
    pub fn respond(self, challenge: &FieldElement) -> UnivarPolyEvalArg {
        ...
        let f_bar = u_raised_to_powers_of_2.iter().zip(self.f.iter()).map(|(u_i, f_i)|((challenge * *u_i) + f_i)).collect::<Vec<FieldElement>>();
        ...
        let r_bar = r.iter().zip(self.s.iter()).map(|(r_i, s_i)|((challenge * r_i) + s_i)).collect::<Vec<FieldElement>>();
        ...
        let xi_bar = self.xi.iter().enumerate().map(|(j, xi)| &(challenge * &r[j+1]) - &(&f_bar[j] * &r[j]) + xi).collect::<Vec<FieldElement>>();
    ...
    fn get_poly_Q(poly_P: &UnivarPolynomial, d: usize, u_raised_to_powers_of_2: &[&FieldElement], f: &[FieldElement]) -> UnivarPolynomial {
        ...
        for i in 0..=two_raised_to_d {
            poly_Q_terms[i] = poly_Q_terms[i].multiply_by_constant(&poly_P[i])
        }
        poly_Q_terms.iter().fold(UnivarPolynomial::new(d), |sum, p| &sum + p)
    }
}
...
impl UnivarPolyEvalArg {
    ...
    fn calculate_delta_bar(poly_P: &UnivarPolynomial, d: usize, x: &FieldElement, f_bar: &[FieldElement]) -> FieldElement {
        ...
        for i in 0..=two_raised_to_d {
            delta_bar_terms[i] = delta_bar_terms[i].multiply_by_constant(&poly_P[i])
        }

        let delta_bar_poly = delta_bar_terms.iter().fold(UnivarPolynomial::new(d), |sum, p| &sum + p);
